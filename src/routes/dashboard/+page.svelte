<script lang="ts">
	...
	const parentData = data as {
		user: { full_name: string; user_type: 'performer' | 'client' | 'both' } | null;
	...
	let showPerformerView = $derived(parentData.isPerformer);
	let showClientView = $derived(parentData.isClient);
</script>
...
<div class="space-y-8">
	...
	<div>
		<h1 class="font-display text-2xl lg:text-3xl font-bold text-secondary">
			Welcome back, {parentData.isPerformer && parentData.performerProfile?.stage_name
				? parentData.performerProfile.stage_name
				: parentData.user?.full_name || 'there'}!
		</h1>
		<p class="text-gray-600 mt-1">
			{#if showPerformerView && showClientView}
				Here's what's happening with your gigs and bookings.
			{:else if showPerformerView}
	...
	<!-- Profile Completion Banner (performers) -->
	{#if showPerformerView && parentData.hasPerformerProfile && !parentData.performerProfile?.profile_complete}
		...
		{:else}
			...
			<div class="bg-white rounded-card shadow-card p-6">
				<div class="flex items-center gap-3">
					...
					<div>
						<p class="text-2xl font-bold text-secondary">{data.clientStats?.pending ?? 0}</p>
			...
			<div class="bg-white rounded-card shadow-card p-6">
				<div class="flex items-center gap-3">
					...
					<div>
						<p class="text-2xl font-bold text-secondary">{data.clientStats?.upcoming ?? 0}</p>
			...
			<div class="bg-white rounded-card shadow-card p-6">
				<div class="flex items-center gap-3">
					...
					<div>
						<p class="text-2xl font-bold text-secondary">{data.clientStats?.completed ?? 0}</p>
						<p class="text-sm text-gray-500">Completed</p>
					</div>
				</div>
			</div>
			...
			<div class="bg-white rounded-card shadow-card p-6">
				<div class="flex items-center gap-3">
					...
					<div>
						<p class="text-2xl font-bold text-secondary">{formatPrice(data.clientStats?.totalSpent ?? 0)}</p>
	...
	<!-- Performance Snapshot (performers only) -->
	{#if showPerformerView}
		...
		<div class="bg-white rounded-card shadow-card">
			<div class="p-6 border-b border-gray-100">
				<div class="flex items-center justify-between">
					...
					<a
						href={showPerformerView ? '/dashboard/bookings' : '/dashboard/my-bookings'}
			...
			<div class="divide-y divide-gray-100">
				{#if data.upcomingEvents && data.upcomingEvents.length > 0}
					{#each data.upcomingEvents as event}
						<a
							href={showPerformerView ? `/dashboard/bookings/${event.id}` : `/dashboard/my-bookings`}
							...
							<div class="flex items-center gap-4">
								...
								<div class="flex-1 min-w-0">
									...
									<p class="text-sm text-gray-500">
										...
										{:else}
											{event.performer?.stage_name || event.performer?.user?.full_name || 'Performer'}
										{/if}
									</p>
								</div>
			...
			<div class="divide-y divide-gray-100">
				{#if data.recentBookings && data.recentBookings.length > 0}
					{#each data.recentBookings as booking}
						<a
							href={showPerformerView ? `/dashboard/bookings/${booking.id}` : `/dashboard/my-bookings`}
							class="block p-4 hover:bg-gray-50 transition-colors"
						>
							<div class="flex items-center gap-4">
								{#if showPerformerView}
	...
	<div class="bg-white rounded-card shadow-card p-6">
		...
		<div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
			...
			{:else}
				<a href="/dashboard/my-bookings" class="p-4 rounded-lg border border-gray-200 hover:border-primary hover:bg-primary/5 transition-colors text-center">
					<svg class="w-8 h-8 text-primary mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z" />
					</svg>
					<span class="text-sm font-medium text-gray-900">My Bookings</span>
				</a>
				<a href="/dashboard/messages" class="p-4 rounded-lg border border-gray-200 hover:border-primary hover:bg-primary/5 transition-colors text-center">
					<svg class="w-8 h-8 text-primary mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
					</svg>
					<span class="text-sm font-medium text-gray-900">Messages</span>
				</a>
				<a href="/dashboard/settings" class="p-4 rounded-lg border border-gray-200 hover:border-primary hover:bg-primary/5 transition-colors text-center">
					<svg class="w-8 h-8 text-primary mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
